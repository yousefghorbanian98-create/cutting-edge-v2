...
class MuscleEnhancer:
    ...
    def enhance_frame(self, frame: np.ndarray, s: EnhancementSettings | None = None) -> np.ndarray:
        ...
        l_strong = clahe_strong.apply(lum)
        l_mild = clahe_mild.apply(lum)
        ...
        blurred = cv2.GaussianBlur(
            gray, (s.dodge_burn_radius * 2 + 1, s.dodge_burn_radius * 2 + 1), s.dodge_burn_radius
        )
        detail = gray - blurred
        ...
        result = np.clip(result.astype(np.float32) + (result.astype(np.float32) - 128) * mid3 * 0.3, 0, 255).astype(
            np.uint8
        )
    ...
    def _create_body_mask(self, frame, h, w):
        ...
        for p1, p2, p3, r in regions:
            pts = np.array(
                [
                    [int(lm[p1].x * w), int(lm[p1].y * h)],
                    [int(lm[p2].x * w), int(lm[p2].y * h)],
                    [int(lm[p3].x * w), int(lm[p3].y * h)],
                ],
                np.int32,
            )
            ...
            for s, e in [(p1, p2), (p2, p3)]:
                cv2.line(
                    mask, (int(lm[s].x * w), int(lm[s].y * h)), (int(lm[e].x * w), int(lm[e].y * h)), 1.0, int(r * w)
                )
