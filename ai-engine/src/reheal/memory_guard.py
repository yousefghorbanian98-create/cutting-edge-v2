    ...
    def put(self, key, data, nbytes):
        with self.lock:
            if key in self.cache:
                self.size -= self.cache[key][1]
                del self.cache[key]
            while self.size + nbytes > self.max_bytes and self.cache:
                _, (_, s) = self.cache.popitem(last=False)
                self.size -= s
            self.cache[key] = (data, nbytes)
            self.size += nbytes
...
class MemoryGuard:
    ...
    def emergency_cleanup(self):
        ...
        except Exception as exc:  # torch missing or no CUDA — guard is best-effort
            logging.getLogger(__name__).debug("cuda cache release skipped: %s", exc)
