...
async def viral_cut(file: UploadFile = File(...), target_duration: int = Form(30)):
    path = save_upload(file)  # validate type/size first (S-003 fail-fast)
...
async def muscle_enhance(file: UploadFile = File(...), intensity: float = Form(0.6), preset: str = Form("natural_gym")):
    path = save_upload(file)  # validate type/size first (S-003 fail-fast)
...
async def style_compare(reference: UploadFile = File(...), source: UploadFile = File(...)):
