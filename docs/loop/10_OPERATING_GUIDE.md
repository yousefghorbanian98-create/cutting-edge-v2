...
```
چت ۱ = ناظر (ثابت، همیشه باز)
چت ۲ = WORKER (سازنده + بازبینِ subagent) — برای هر ۱–۲ مرحله یک چت جدید
```
کاربر فقط: چت ۲ را می‌سازد → پرامپت WORKER را می‌فرستد → جواب آخر را به چت ۱ می‌آورد و می‌نویسد «چک کن».
ناظر هر وقت تشخیص دهد بازبینی داخل چت مستقل نبوده، خودش یک بازبین جدا می‌خواهد (بخش B).
