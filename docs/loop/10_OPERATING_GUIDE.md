...
## B1. چت سازنده ثابت می‌ماند — مگر ناظر بگوید «چت جدید»
...
```
Sync and continue:
  git fetch origin arena/01a0c936-cutting-edge-v2 && git merge --ff-only FETCH_HEAD
...
**اگر توکن GitHub چت سازنده منقضی شد** (پیام «authentication failed»): اول در همان چت GitHub را در Arena دوباره وصل کنید و فقط بنویسید `push`. کامیت‌های push‌نشده فقط در سندباکس آن چت هستند؛ اگر چت را رها کنید از دست می‌روند. چت جدید همیشه از شاخه‌ی ناظر ساخته می‌شود: `arena/01a0c936-cutting-edge-v2`.

## B2. انتقال چت سازنده به یک نشست جدید (وقتی چت سنگین/کند شده)

چت سازنده بعد از چند مرحله سنگین می‌شود (کانتکست بلند = لود کند + خطای بیشتر). انتقال سه قدم دارد و **هیچ کاری از دست نمی‌رود** چون همه‌چیز روی GitHub است:

1. **در چت قدیمی** بنویسید: `handover` — سازنده همه‌چیز را commit/push می‌کند، SHA نهایی را با `git ls-remote` تأیید می‌کند و یک خط «وضعیت انتقال» در `docs/loop/evidence/SESSIONS.md` می‌نویسد. اگر PUSH_OK نداد، چت قدیمی را نبندید (کامیت‌های push‌نشده فقط در سندباکس آن چت هستند).
2. **چت جدید** روی همین مخزن باز کنید (Arena شاخهٔ تازه‌ای به شکل `arena/<id>-cutting-edge-v2` از `main` می‌سازد — `main` فقط کامیت اولیه است، پس چت جدید باید خودش شاخهٔ سازندهٔ قبلی را روی شاخهٔ خودش بیاورد). پرامپت «BUILDER HANDOVER» از `07_SESSION_HANDOFF.md` را بفرستید و به‌جای `<OLD_BRANCH>` نام شاخهٔ قبلی و به‌جای `<OLD_SHA>` آخرین SHA را بگذارید.
3. چت جدید بعد از هم‌ترازی، نام شاخهٔ مرجع را در اسناد زنده عوض می‌کند و یک پیام آماده برای چت ناظر می‌دهد (`شاخهٔ سازنده عوض شد: arena/<id>-cutting-edge-v2 @ <sha>`). آن را در چت ناظر بفرستید. از این به بعد چت قدیمی را استفاده نکنید.

از این به بعد شاخهٔ مرجع = شاخهٔ چت سازندهٔ **فعلی** (در بالای `00_INDEX.md` نوشته می‌شود). فایل‌های شواهد و learnings قدیمی نام شاخهٔ قبلی را به‌عنوان تاریخچه نگه می‌دارند و بازنویسی نمی‌شوند.
