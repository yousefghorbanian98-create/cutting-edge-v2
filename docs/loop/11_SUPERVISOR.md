...
## ۶. ناظر به‌عنوان بازبین مستقل (حالت عملیاتی تا ۲۲ سپتامبر ۲۰۲۶ — از آن پس: چت دوم، `14_OVERSEER.md`)
...
## 7. حلقه‌ی improve (از ECC — پذیرفته‌شده در `13_INTEGRATIONS_ADOPTION.md`)
در هر ممیزی («چک کن») ناظر علاوه بر ۱۴ چک:
1. ورودی‌های جدید `docs/learnings/` را می‌خواند (C13 وجودشان را برای هر سشن سازنده الزامی می‌کند — از S-101).
2. برای هر درس دقیقاً یکی از این چهار خروجی را تولید می‌کند و در گزارش `evidence/SUPERVISOR/<date>.md` بخش «Improve» ثبت می‌کند:
   - چک جدید/سخت‌تر در `scripts/supervise.py` یا `verify_ledger.py`؛
   - قاعده‌ی جدید در `02_LOOP_PROTOCOL.md` (با شماره‌ی گام)؛
   - تغییر در پرامپت `07_SESSION_HANDOFF.md`؛
   - کارت جدید/تقویت‌شده در `steps.json`.
3. اگر درس تکراری است (همان ریشه دو بار)، **باید** به چک خودکار تبدیل شود — نه قاعده‌ی متنی.
نمونه‌های تا امروز: انقضای توکن → گیت پوش در ①؛ کامیت چندمرحله‌ای → C4؛ نبود مرورگر/کارگو → قاعده‌ی `unverified:ci` در پرامپت batch؛ فیکسچر با پسوند اشتباه → S-005 r2.

## 8. چک‌های C15 و C16 (از ممیزی خارجی ۲۰۲۶-۰۹-۰۸ — ADR-0002)
- **C15 Stack-Fit:** هر خط وابستگی جدید در `pyproject.toml`، `requirements.txt`، `package.json`‌ها و `Cargo.toml` که با الگوی استک بیگانه (ORM/سرور/احراز هویت/چت‌فریم‌ورک/…) بخواند → FAIL. راه عبور: ابتدا ADR + تغییر `steps.json`، سپس وابستگی.
- **C16 License-Fit:** `scripts/license_check.py` (از S-086) با allow-list OSI روی هر سه اکوسیستم؛ تا آن زمان WARN.
