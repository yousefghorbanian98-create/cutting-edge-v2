...
| S-010 | Tauri walking skeleton that compiles and ships a first .exe | REVIEW | 1 | local-linux | docs/loop/evidence/S-010/CONTRACT.md; tests/unit/test_tauri_skeleton.py (10 pass + 1 win-only); gate static 13/0/0/1 (new `icons` check) | awaiting `ci / windows`: cargo fmt/clippy/test hard, `tauri build` → NSIS, installer_smoke.ps1, Cargo.lock capture (AC-7) |
