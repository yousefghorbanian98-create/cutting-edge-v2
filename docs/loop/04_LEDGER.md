...
| S-010 | Tauri walking skeleton that compiles and ships a first .exe | REVIEW | 1 | local-linux; ci-windows | docs/loop/evidence/S-010/CONTRACT.md; EVIDENCE.md; tests/unit/test_tauri_skeleton.py (10 pass + 1 win-only); gate static 13/0/0/1 (new `icons` check); CI run 35711041909 ✅; `--locked` proof CI run 35734652629 ✅ (job 106768677907: clippy/test/tauri `--locked`, smoke 17/17, installer sha256 `70A3A741…E1E1`) | awaiting Overseer verdict; AC-7 lock `40efd7b` sha256 `01B0A06D…34A6`; BUG-17 CLOSED on 390b6ca; NSIS Persian = NSIS name `Farsi` + custom `nsis/Farsi.nsh` |
| S-011 | Loop tooling: gate.py stages, ledger verifier, smoke-gpu.ps1, session handoff template | REVIEW | 1 | local-linux | docs/loop/evidence/S-011/CONTRACT.md; EVIDENCE.md; tests/unit/test_loop_tooling.py (9 pass local-linux) | awaiting Overseer. `gate.py --stage all` reports static/unit/real/e2e/perf/chaos as PASS/FAIL/MISSING (never empty). unit runs `pytest tests/unit -m unit` (PASS); vitest and cargo-test MISSING with a reason. real MISSING locally (`cv2` not installed). e2e/perf/chaos MISSING owned by S-079/S-082/S-077. `scripts/smoke-gpu.ps1` always prints JSON; `-DryRun` is wired on `ci / windows` (unverified:windows until that job). Handoff template locked. Not GREEN. |
