...
| S-010 | Tauri walking skeleton that compiles and ships a first .exe | REVIEW | 1 | local-linux; ci-windows | docs/loop/evidence/S-010/CONTRACT.md; EVIDENCE.md; tests/unit/test_tauri_skeleton.py (10 pass + 1 win-only); gate static 13/0/0/1 (new `icons` check); CI run 35711041909 ✅ (windows job 106691418725: fmt/clippy/test hard, `tauri build` → `Cutting Edge_2.0.0_x64-setup.exe` 2.02 MB, installer smoke 17/17, artifact `cutting-edge-windows-x64-setup`) | awaiting Overseer verdict; AC-7 lock committed `40efd7b` sha256 `01B0A06D…34A6` (114995 B, matches run 35711041909); `--locked` wired in the follow-up (awaiting ci/windows); NSIS Persian = NSIS name `Farsi` + custom `nsis/Farsi.nsh` |
