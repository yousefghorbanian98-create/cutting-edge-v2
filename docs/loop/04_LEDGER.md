...
| S-011 | Loop tooling: gate.py stages, ledger verifier, smoke-gpu.ps1, session handoff template | REVIEW | 2 | local-linux; ci-windows | docs/loop/evidence/S-011/CONTRACT.md; EVIDENCE.md; REVIEW.md (changes-requested r1 @ f082e8f); tests/unit/test_loop_tooling.py | round 2. CI run 35986178547 on 93ede82: smoke-gpu.ps1 -DryRun succeeded, then pytest failed and skipped cargo/installer, so the windows job is not green. Not GREEN. |
| S-012 | Non-blocking processing: job model + thread pool so /health stays alive | REVIEW | 2 | local-linux | docs/loop/evidence/S-012/CONTRACT.md; EVIDENCE.md; REVIEW.md (changes-requested r1 @ e0d2dfb); tests/test_jobs.py; tests/unit/test_job_scheduler.py; tests/test_api_live.py | round 2. CI run 35986178547 windows pytest failed: test_muscle_enhance_live_http expected 200, got 202. Live helper now polls the job. Not GREEN. BUG-11 stays OPEN until this SHA's ci/windows is green. |
