...
| S-003 | Security fix: upload/download path traversal, size & type limits, CORS | REVIEW | 1 |  | docs/loop/evidence/S-003/CONTRACT.md; tests/test_security.py (6/6 pass real, local-linux) | Ready for fresh review. Storage (UUID names, extension whitelist, streaming size limit, path-traversal-safe resolve) + restricted CORS + fail-fast save_upload before heavy imports. Ruff/Bandit deferred to S-008; applicable static checks (verify_ledger.py, py_compile) green. Size limit configurable via CE_MAX_UPLOAD_MB (default 2GB). |
| S-004 | MoviePy 2.0 compatibility + FFmpeg-first audio extraction (BUG 4) | REVIEW | 1 |  | docs/loop/evidence/S-004/CONTRACT.md; tests/test_beat_sync.py (3/3 pass real, local-linux) | BUG-4 closed. core/ffmpeg.py (discover CE_FFMPEG_BIN→PATH→imageio-ffmpeg; ffmpeg subprocess primary, MoviePy 2 import only fallback); beat_sync returns [] on silence (no dummy beats). BPM 117.45 (±3 of 120) on click-track MP4; silent MP4 → []; AAC 48k → WAV 22050 mono. ffprobe absent in sandbox → WAV validated via wave module + ffmpeg -i. Ruff/Bandit deferred to S-008. |
