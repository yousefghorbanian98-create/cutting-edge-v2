...
| S-012 | Non-blocking processing: job model + thread pool so /health stays alive | REVIEW | 1 | local-linux | docs/loop/evidence/S-012/CONTRACT.md; EVIDENCE.md; tests/test_jobs.py (2 passed, 60.12s); tests/unit/test_job_scheduler.py (4 passed) | awaiting Overseer. Not GREEN. BUG-11 stays OPEN until ci/windows. GPU slot is one inference job; this host `probe_gpu` is false, so GPU is `unverified`, not a pass. No Style Match model installed. |
