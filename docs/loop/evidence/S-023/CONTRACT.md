...
- Evidence plan: vitest دامنه پیش از integration. بعد از static و unit و typecheck، Playwright. پیش از تحویل batch، CI کامل Ubuntu و Windows. local pass جایگزین CI نیست. آستانهٔ `±1px` شل نمی‌شود. عرض قفل نشانگر باید در state بماند. ستون هدر داخل `scrollWidth` است؛ fit باید خط را در `client - header` جا بدهد. CI `36125925821` مقدار `1582` را در برابر `1407` گرفت. CI `36128799735` مقدار `1617` را گرفت، یعنی هدر اندازه‌گیری نشد و ستون واقعی `211` پیکسل بود. selector تست عوض نمی‌شود.
