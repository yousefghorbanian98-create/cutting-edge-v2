...
- Registry IDs: `K-004` برای نگاشت زمان و ممنوعیت mutation مستقیم. `K-005` اسکرین‌شات را جایگزین assertion پخش نمی‌کند. `K-006` آستانهٔ یک فریم را شل نمی‌کند. `K-007` اینجا اعمال نمی‌شود. `K-008` مجوز ادامه نیست. `K-011` رد می‌شود. `K-001` فقط به‌عنوان رد raw command؛ اجرای FFmpeg این مرحله نیست. `K-012` و `K-013` اینجا اجرا نمی‌شوند. `K-014` ابزار دات‌نت و گیت تکراری را رد می‌کند.
...
- Evidence note: CI `36120355492` فاصلهٔ پلی‌هد را `0.0259s` در برابر `< 1/60` گرفت. CI `36136231456` مقدار `0.0182s` را گرفت. آستانه شل نمی‌شود. منحنی یک‌ثانیه‌ای یک vsync از `video.currentTime` جلو بود. پایه حالا خود ساعت رسانه است و دنبالهٔ انیمیشن نیم‌فریم در `60` است، نه یک ثانیه. شکاف تا evidence نام‌دار باز می‌ماند.
