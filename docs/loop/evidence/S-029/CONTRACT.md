...
- Registry IDs: `K-001` یعنی پیام سوکت raw command اجرا نمی‌کند. `K-004` UI را از mutation مستقیم sequence جدا می‌کند. `K-006` درصد ساختگی را رد می‌کند. `K-007` اعمال نمی‌شود. `K-008` مجوز ادامه نیست. `K-011` رد می‌شود. `K-002` همچنان owner خودش را دارد و اینجا بسته نمی‌شود. `K-012` و `K-013` اینجا اجرا نمی‌شوند. `K-014` گیت دات‌نت اضافه نمی‌کند.
- Open gaps: `BUG-18` و `user-gpu` باز می‌مانند. موفقیت job ویندوز شکست Ubuntu را پاک نمی‌کند.
