...
- Registry IDs: `K-001` یعنی پیام سوکت raw command اجرا نمی‌کند. `K-004` UI را از mutation مستقیم sequence جدا می‌کند. `K-006` درصد ساختگی را رد می‌کند. `K-007` اعمال نمی‌شود. `K-008` مجوز ادامه نیست. `K-011` رد می‌شود. `K-002` همچنان owner خودش را دارد و اینجا بسته نمی‌شود.
