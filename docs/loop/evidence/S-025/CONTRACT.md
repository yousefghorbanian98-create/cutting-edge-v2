...
- Registry IDs: `K-004` چون عمل شورتکات از store action می‌گذرد. `K-006` برای تعامل واقعی و RTL. `K-005` مرجع دیداری است، نه پذیرش. `K-007` اعمال نمی‌شود. `K-008` مجوز ادامه نیست. `K-011` رد می‌شود. `K-001` تا `K-003` و `K-009` و `K-010` اینجا اجرا نمی‌شوند. `K-012` و `K-013` اینجا اجرا نمی‌شوند. `K-014` ابزار دات‌نت و گیت تکراری را رد می‌کند.
