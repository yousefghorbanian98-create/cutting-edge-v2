...
- Registry IDs: `K-001` بدنه را typed می‌کند. `K-002` overwrite پیش‌فرض را false نگه می‌دارد و `BUG-18` را نمی‌بندد. `K-006` هشدار 4K را پنهان نمی‌کند. `K-007` اعمال نمی‌شود. `K-008` مجوز ادامه نیست. `K-011` رد می‌شود. `K-012` و `K-013` اینجا اجرا نمی‌شوند. `K-014` گیت دات‌نت اضافه نمی‌کند.
