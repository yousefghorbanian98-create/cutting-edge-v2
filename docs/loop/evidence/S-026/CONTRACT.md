...
- Registry IDs: `K-004` چون چیدن و undo از store action است. `K-005` تفاوت بصری مشاهده است و آستانهٔ `0.001` را جایگزین نمی‌کند. `K-006` سفر واقعی را به helper خالص ترجیح می‌دهد. `K-007` اعمال نمی‌شود. `K-008` مجوز ادامه نیست. `K-011` رد می‌شود. `K-012` و `K-013` اینجا اجرا نمی‌شوند. `K-014` ابزار دات‌نت و گیت تکراری را رد می‌کند.
